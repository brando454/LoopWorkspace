...
final class ECJPAKEContext {
    ...
    private var X3: ECPoint?
    private var X4: ECPoint?
    ...
    func clientRound1() throws -> Data {
        let X1 = ECPoint.generator.multiplied(by: x1)
        let X2 = ECPoint.generator.multiplied(by: x2)
    ...
    struct ZKP {
        let V: ECPoint   // commitment point
        let b: Fq        // response scalar
    }

    // Prove knowledge of x such that X = x * G
    private func makeZKP(x: Fq, X: ECPoint, generator: ECPoint) -> ZKP {
        let v = Fq.random()
    ...
    private static func verifyZKP(_ zkp: ZKP, X: ECPoint, generator: ECPoint, signerID: Data?) -> Bool {
    ...
    private static func zkpChallenge(generator: ECPoint, V: ECPoint, X: ECPoint, signerID: Data) -> Fq {
        var data = Data()
    ...
    private func zkpChallenge(generator: ECPoint, V: ECPoint, X: ECPoint, signerID: Data) -> Fq {
    ...
    private func encodeChunk(point: ECPoint, zkp: ZKP) -> Data {
        var d = Data()
    ...
    private func decodeAndVerifyChunk(_ data: Data, generator: ECPoint, peerID: Data?) throws -> (ECPoint, ZKP) {
        ...
        guard let X = ECPoint(x963: Data(pointBytes)),
              let V = ECPoint(x963: Data(vBytes)),
